 ...
 * ───────────────────────────────────────
 * Map tab — now includes a "Test API" button in top-right
 * so you can quickly diagnose connection issues.
 * Remove the button once everything is working.
 */


// Comment what is below uncomment what is above , paste below code to index.tsx
import React, { useCallback, useRef, useState } from "react";
import {
  ActivityIndicator,
  Modal,
...
export default function MapScreen() {
  ...
  const { location, hasPermission, isLoading: locLoading } = useLocation();
  const { blackspots, isLoading: spotsLoading, error: spotsError, refetch } = useBlackspots();
  const { alertState, dismissAlert } = useAlert(location);

  const [selectedSpot,  setSelectedSpot]  = useState<BlackSpot | null>(null);
  const [filterLevel,   setFilterLevel]   = useState<"ALL"|"HIGH"|"MEDIUM"|"LOW">("ALL");

  // Navigate to alert screen when HIGH
  React.useEffect(() => {
    if (alertState.active && alertState.level === "HIGH") {
  ...
  // Centre map on user when first acquired
  React.useEffect(() => {
    if (location && mapRef.current) {
      mapRef.current.animateToRegion({
        latitude:       location.latitude,
  ...
  const zoneRadius = (level: string) =>
    level === "HIGH" ? CONFIG.HIGH_ZONE_RADIUS_M
    : level === "MEDIUM" ? CONFIG.MEDIUM_ZONE_RADIUS_M
    : CONFIG.LOW_ZONE_RADIUS_M;

  if (!hasPermission && !locLoading) {
    return (
      <View style={styles.centred}>
        <Text style={styles.permText}>📍 Location permission required</Text>
        <Text style={styles.permSub}>Enable in Settings → App → Location</Text>
      </View>
    );
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <AlertBanner
        alertState={alertState}
        onDismiss={dismissAlert}
        onTap={() => router.push("/screens/AlertScreen" as any)}
      />
      ...
      <View style={styles.filterBar}>
        {(["ALL","HIGH","MEDIUM","LOW"] as const).map((f) => (
          <TouchableOpacity
            ...
            onPress={() => setFilterLevel(f)}
          >
            <Text style={[styles.filterText, filterLevel === f && styles.filterTextActive]}>
              {f}
            </Text>
          </TouchableOpacity>
        ))}
        ...
        {/* Debug button — remove after confirming API works */}
        <TouchableOpacity
          style={styles.debugBtn}
          onPress={() => router.push("/screens/ConnectionTest" as any)}
        >
      ...
      {spotsError && (
        <View style={styles.errorBar}>
          <Text style={styles.errorBarText}>
            ⚠ API Error: {spotsError} — tap 🔌 to diagnose
          </Text>
        </View>
      )}
          ...
          return (
            <React.Fragment key={spot.cluster_id}>
              ...
              <Marker
                ...
                anchor={{ x: 0.5, y: 0.5 }}
              >
                <View style={[styles.zonePin, { borderColor: mc.stroke }]}>
                  <Text style={[styles.zonePinText, { color: mc.stroke }]}>
                    {spot.risk_level === "HIGH" ? "⚠" : spot.risk_level === "MEDIUM" ? "◆" : "●"}
                  </Text>
                </View>
              </Marker>
            </React.Fragment>
      ...
      <View style={styles.countBadge}>
        <Text style={styles.countText}>
          {spotsLoading ? "Loading..." : `${visibleSpots.length} zones`}
        </Text>
      </View>
      ...
      {(locLoading || spotsLoading) && (
        <View style={styles.loadingOverlay}>
          <ActivityIndicator color={COLORS.accent} size="large" />
          <Text style={styles.loadingText}>
            {locLoading ? "Getting GPS..." : "Loading blackspots from API..."}
          </Text>
        </View>
      )}
        ...
        >
          ...
          <View style={styles.sheet}>
            ...
            <ScrollView>
              ...
              <TouchableOpacity
                style={styles.routeBtn}
                onPress={() => { setSelectedSpot(null); router.push("/(tabs)/route"); }}
              >
                <Text style={styles.routeBtnText}>📍 Plan Route Here</Text>
              </TouchableOpacity>
            </ScrollView>
...
const styles = StyleSheet.create({
  ...
  centred:    { flex: 1, justifyContent: "center", alignItems: "center",
                backgroundColor: COLORS.background, padding: 32 },
  permText:   { color: COLORS.textPrimary, fontSize: 18, fontWeight: "700", textAlign: "center" },
  permSub:    { color: COLORS.textSecondary, fontSize: 13, textAlign: "center", marginTop: 8 },

  filterBar:  { flexDirection: "row", paddingHorizontal: 10, paddingVertical: 8,
                backgroundColor: COLORS.surface, gap: 6,
                borderBottomWidth: 1, borderColor: COLORS.border, flexWrap: "nowrap" },
  filterBtn:  { paddingHorizontal: 10, paddingVertical: 5, borderRadius: 16,
  ...
  refreshText: { color: COLORS.accent, fontSize: 18 },
  debugBtn:    { padding: 6, marginLeft: "auto" as any },
  debugText:   { fontSize: 18 },

  errorBar:     { backgroundColor: COLORS.highBg, padding: 8, borderBottomWidth: 1,
                  borderColor: COLORS.high },
  errorBarText: { color: COLORS.high, fontSize: 11, textAlign: "center" },

  zonePin:     { width: 28, height: 28, borderRadius: 14, borderWidth: 2,
                 backgroundColor: "rgba(10,14,26,0.75)", justifyContent: "center",
                 alignItems: "center" },
  zonePinText: { fontSize: 12, fontWeight: "700" },
  ...
  loadingOverlay: { ...StyleSheet.absoluteFillObject, backgroundColor: "rgba(10,14,26,0.75)",
                    justifyContent: "center", alignItems: "center", gap: 12 },
  loadingText:    { color: COLORS.textSecondary, fontSize: 13 },

  sheetBackdrop: { flex: 1, backgroundColor: "rgba(0,0,0,0.5)" },
  sheet:         { backgroundColor: COLORS.surface, borderTopLeftRadius: 20,
                   borderTopRightRadius: 20, padding: 16, maxHeight: "55%",
                   borderTopWidth: 1, borderColor: COLORS.border },
  sheetHandle:   { width: 40, height: 4, backgroundColor: COLORS.border,
                   borderRadius: 2, alignSelf: "center", marginBottom: 16 },
  routeBtn:      { backgroundColor: COLORS.accentBg, borderWidth: 1, borderColor: COLORS.accent,
                   borderRadius: 10, padding: 14, alignItems: "center", marginTop: 12 },
  routeBtnText:  { color: COLORS.accent, fontWeight: "700", fontSize: 14 },
});